			       ...
			       const void *data, uint16_t length)
{
	...
	if (err) {
		...
		if (length == OTS_FEATURE_LEN) {
			...
			BT_DBG("Invalid length %u (expected %lu)",
				      ...
				      const void *data, uint16_t length)
{
	...
	if (err) {
		...
		BT_WARN("Invalid length %u (expected %lu)",
			 ...
			 struct dirlisting_record_t *rec)
{
	...
	if (BT_OTS_DIRLIST_GET_FLAG_TYPE_128(rec->flags)) {
		...
		if ((start_len - buf->len) + BT_UUID128_SIZE > rec->len) {
		...
		uuid = net_buf_simple_pull_mem(buf, BT_UUID128_SIZE);
		bt_uuid_create(&rec->metadata.type_uuid.uuid,
			       uuid, BT_UUID128_SIZE);
	} else {
		if ((start_len - buf->len) + BT_UUID16_SIZE > rec->len) {
			BT_WARN("incorrect DirListing record, reclen %u "
